...
object StepService : SensorEventListener {
    ...
    /** Same human-plausible per-bucket cap as MetricsSampler: a bigger jump is a glitching counter
     *  (e.g. reporting its raw cumulative-since-boot value), so re-baseline instead of injecting a spike. */
    ...
    @Synchronized override fun onSensorChanged(sensorEvent: SensorEvent?) {
        ...
        if (previousStepCount >= 0) {
            val delta = stepCount - previousStepCount
            // Ignore a reset (negative) or a garbage forward jump; just re-baseline via previousStepCount below.
            if (delta in 0..MAX_PLAUSIBLE_BUCKET_STEPS) {
                var recentStepCount = delta
