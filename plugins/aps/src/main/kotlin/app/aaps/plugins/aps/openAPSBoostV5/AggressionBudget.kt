...
fun aggressionBudget(
    ...
    // Defensive: a non-finite baseInsulinReq must never reach the dose math — clamp to 0.0.
    val baseReq = if (baseInsulinReq.isFinite()) baseInsulinReq else 0.0
    ...
    val rawBudget = baseReq * aggressionModifier
    val floorBudget = BUDGET_FLOOR_FRACTION * baseReq
...
internal fun mlHypoRiskScale(mlHypoRisk: Double?, hypoCautionKnob: Double = 1.0): Double {
    // Defensive: a non-finite risk (NaN/Inf from a degenerate feature) must not propagate into the
    // dose multiplier — treat it as "no signal" (scale 1.0), same as a missing model.
    if (mlHypoRisk == null || !mlHypoRisk.isFinite()) return 1.0
