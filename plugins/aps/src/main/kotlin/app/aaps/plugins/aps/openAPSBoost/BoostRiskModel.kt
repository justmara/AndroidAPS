...
class BoostRiskModel @Inject constructor(
    ...
    private fun walkTree(node: TreeNode, features: DoubleArray): Double {
        ...
        // Defensive: a malformed model whose split references an out-of-range feature index must not
        // throw on the dosing path. Treat a missing/non-finite value as "> threshold" (right branch),
        // the same branch NaN already takes, so a bad model degrades instead of crashing.
        val v = features.getOrNull(node.featureIndex) ?: Double.NaN
        return if (v.isFinite() && v <= node.threshold) {
