...
open class OpenAPSBoostV2Plugin @Inject constructor(
    ...
    private fun calculateBoostActivity(
        ...
        if (boostActive && tempTargetSet && !allowBoostWithHighTt && targetBg > dynIsfNormalTarget) {
            boostActive = false
            disableReason = "High temp target (${Math.round(targetBg)} > ${Math.round(dynIsfNormalTarget)})"
            aapsLogger.debug(LTag.APS, "Boost disabled due to high temptarget of $targetBg")
        }

        val recentStepsWindows = WearStepSource.recentWindowsOrFallback(persistenceLayer, dateUtil.now(), preferences.get(BooleanKey.OverviewAlwaysUsePhoneSteps))
        val recentSteps5Min = recentStepsWindows.m5
        val recentSteps15Min = recentStepsWindows.m15
        val recentSteps30Min = recentStepsWindows.m30
        val recentSteps60Min = recentStepsWindows.m60
    ...
    override fun invoke(initiator: String, tempBasalFallback: Boolean) {
        ...
        // 7. Step counts — watch-first, phone-pedometer fallback
        val recentStepsWindows = WearStepSource.recentWindowsOrFallback(persistenceLayer, dateUtil.now(), preferences.get(BooleanKey.OverviewAlwaysUsePhoneSteps))
        val recentSteps5Min = recentStepsWindows.m5
        val recentSteps15Min = recentStepsWindows.m15
        val recentSteps30Min = recentStepsWindows.m30
        ...
        // ---- Dynamic / Auto Carb Ratio (ported from iAPS/Trio) ----
        // 0.0 = disabled (DetermineBasal uses profile.carb_ratio). Boost always runs dynamic ISF, so the
        // formula uses Boost's dynamic ISF (isfResult.variableSens) and AutoCR mirrors it to keep CSF (ISF/CR)
        // at the profile baseline. Priority: formula-based Dynamic CR over AutoCR when both are enabled.
        ...
        run {
            ...
            if (preferences.get(BooleanKey.ApsUseDynamicCarbRatio)) {
                ...
                // AutoCR — follow Boost's dynamic ISF adjustment, keeping CSF (ISF/CR) at the profile baseline
                val rr = dynamicCarbRatioCalculator.computeAutoCarbRatio(baseCR, profileIsf, isfResult.variableSens)
                dynamicCarbRatio = rr.carbRatio
                dynamicCarbRatioReason = rr.reason
            }
            // Surface the resulting carb-sensitivity factor (CSF = ISF/CR) vs the profile baseline, so the
            // Boost + Dynamic CR combination's consistency (CSF drift) is visible in Script debug.
            if (dynamicCarbRatio > 0.0)
                dynamicCarbRatioReason += " | CSF ${Round.roundTo(isfResult.variableSens / dynamicCarbRatio, 0.01)} (profile ${Round.roundTo(profileIsf / baseCR, 0.01)})"
        }
    ...
    override fun addPreferenceScreen(preferenceManager: PreferenceManager, parent: PreferenceScreen, context: Context, requiredKey: String?) {
        if (requiredKey != null &&
            ...
            requiredKey != "openapsboostv2_dynamic_cr_settings"
        ) return
        ...
        category.apply {
            ...
            addPreference(preferenceManager.createPreferenceScreen(context).apply {
                ...
                addPreference(AdaptiveSwitchPreference(ctx = context, booleanKey = BooleanKey.DynIsfUseTdd, summary = R.string.boost_use_tdd_v2_summary, title = R.string.boost_use_tdd_title))
                ...
                addPreference(preferenceManager.createPreferenceScreen(context).apply {
                    key = "openapsboostv2_dynamic_cr_settings"
