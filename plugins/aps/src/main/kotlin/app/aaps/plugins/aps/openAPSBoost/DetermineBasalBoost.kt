...
class DetermineBasalBoost @Inject constructor(
    ...
    fun determine_basal(
        ...
        consoleError.add("Effective CR (unusable): $eRatio")
        ...
        // Dynamic / Auto Carb Ratio (ported from Boost V2): when the plugin computed a dynamic CR
        // (profile.dynamicCarbRatio > 0), it replaces profile.carb_ratio everywhere CR drives dosing
        // (CSF, COB→insulin, SMB CR). 0.0 = disabled → use the profile CR. The reason line names the
        // formula (logarithmic/sigmoid/AutoCR) and the resulting ratio so Script debug shows the choice.
        ...
        rT.carbRatio = round(carbRatio, 3)
        rT.carbRatioReason = profile.dynamicCarbRatioReason.ifEmpty { null }

        // Boost uses profile.sens for CSF (not autosens-adjusted)
        val csf = profile.sens / carbRatio
        ...
        rT.reason.append(
            "COB: ${round(meal_data.mealCOB, 1).withoutZeros()}, Dev: ${convert_bg(deviation.toDouble())}, BGI: ${convert_bg(bgi)}, ISF: ${convert_bg(sens)}, CR: ${
                round(carbRatio, 2).withoutZeros()
