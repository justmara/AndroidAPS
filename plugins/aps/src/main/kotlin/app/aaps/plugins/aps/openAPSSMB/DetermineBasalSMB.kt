    ...
    fun determine_basal(
        ...
        iobArray.forEach { iobTick ->
            ...
            val IOBpredBGI: Double =
                if (dynIsfMode && profile.use_TDD_for_predictions == true) round((-iobTick.activity * dynIsfCalculator.getIsfForBg(max(IOBpredBGs[IOBpredBGs.size - 1], 39.0), profile.sensNormalTarget, profile.insulinDivisor, true) * 5), 2)
                else predBGI
            ...
            val predZTBGI =
                if (dynIsfMode && profile.use_TDD_for_predictions == true) round((-iobTick.iobWithZeroTemp!!.activity * dynIsfCalculator.getIsfForBg(max(ZTpredBGs[ZTpredBGs.size - 1], 39.0), profile.sensNormalTarget, profile.insulinDivisor, true) * 5), 2)
                else round((-iobTick.iobWithZeroTemp!!.activity * sens * 5), 2)
            val predUAMBGI =
                if (dynIsfMode && profile.use_TDD_for_predictions == true) round((-iobTick.activity * dynIsfCalculator.getIsfForBg(max(UAMpredBGs[UAMpredBGs.size - 1], 39.0), profile.sensNormalTarget, profile.insulinDivisor, true) * 5), 2)
                else predBGI
        ...
        if (dynIsfMode && profile.use_TDD_for_predictions == true) {
            if (bg > target_bg && glucose_status.delta < 3 && glucose_status.delta > -3 && glucose_status.shortAvgDelta > -3 && glucose_status.shortAvgDelta < 3 && eventualBG > target_bg && eventualBG
                ...
                future_sens = dynIsfCalculator.getIsfForBg(((fSensBG * 0.5) + (bg * 0.5)), profile.sensNormalTarget, profile.insulinDivisor, false)
                ...
                future_sens = dynIsfCalculator.getIsfForBg(fSensBG, profile.sensNormalTarget, profile.insulinDivisor, false)
                future_sens = round(future_sens, 1)
