...
internal interface CarbsDao : TraceableDao<Carbs> {
    ...
    @Query("SELECT * FROM $TABLE_CARBS WHERE likely(isValid = 1) AND unlikely(timestamp >= :from) AND unlikely(timestamp <= :to) AND unlikely(amount > :minCOB) AND likely(referenceId IS NULL) ORDER BY id DESC")
